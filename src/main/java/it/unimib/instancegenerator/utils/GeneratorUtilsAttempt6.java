...
@Component("Utils6")
public class GeneratorUtilsAttempt6 extends GeneratorUtilsAttempt2 {
    ...
    public Knapsack generateRandomKnapsack(int knapsakid, int numKnapsack, List<BigDecimal> fillRatio, List<Family> families, int dim) {
        ...
        IntStream.range(0, dim)
                .mapToObj(j -> new Pair(j, items.stream().mapToInt(i -> i.getWeights().get(j)).boxed().collect(Collectors.toList())))
    ...
    public List<Knapsack> generateNKnapsacks(int numKnapsack, List<Family> families, int dim) {
        return IntStream.range(1, numKnapsack + 1)
                .mapToObj(knapsackId -> generateRandomKnapsack(knapsackId, numKnapsack,
                        IntStream.range(1, dim + 1).mapToObj(i -> generateRandomAlpha()).collect(Collectors.toList()),
                        families, dim))
                .collect(Collectors.toList());
    }
